...
)
{
	...
	spin_lock_irq(&programs_buffer[ids_index].spinlock);
	...
	if (queue_size >= (C3_PROGRAM_QUEUE_SIZE - 1)) {
		...
		spin_unlock_irq(&programs_buffer[ids_index].spinlock);
...
)
{
	...
	} /* if (program_size > C3_PROGRAM_MEMORY_MAX_SIZE) */
...
static int __init c3_init(void)
{
	...
	for (j = 0; j < C3_IDS_NUMBER; j++) {
		...
		for (i = 0; i < C3_PROGRAM_QUEUE_SIZE; i++) {
			...
			sema_init(&programs_buffer[j].program_queue[i].sem, 0);
	...
	if (c3_GP_init() != C3_OK) {
		printk(C3_KERN_ERR "Cannot init GP channel\n");
		goto failure;
	}
...
#ifdef CONFIG_C3_AUTOTEST_DRIVER
	if (c3_autotest() != C3_OK) {
		printk(C3_KERN_ERR "AUTOTEST failed\n");
		goto failure;

	} /* if (c3_autotest_local_memory() != C3_OK)*/
#endif
