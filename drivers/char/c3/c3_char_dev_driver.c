...
static unsigned char __debug_msg[C3_GP_DEBUG_BUF_SIZE];
	...
	unsigned char *user_buffer, unsigned int len, c3_map_info_t *map_info)
{
	...
	map_info->hw_scatter_list = kmalloc((nr_pages)
		* sizeof(c3_hw_scatter_list_element_t), GFP_KERNEL);

#ifdef DEBUG
	printk(
		C3_KERN_DEBUG "[CDD] HW scatter list address: %08x\n",
		(unsigned int)map_info->hw_scatter_list);
#endif
	...
	for (i = 0; i < nr_pages; i++) {
		...
		map_info->hw_scatter_list[i].address = page_to_phys(pages[i])
			+ (i == 0 ? (((unsigned int) user_buffer) & ~PAGE_MASK)
				: 0);
		...
		printk(
			C3_KERN_DEBUG "[CDD]  - Address: %08x\n",
			map_info->hw_scatter_list[i].address);

		printk(
			C3_KERN_DEBUG "[CDD]  - Size: %0d\n",
...
} /* static unsigned int _c3_map_user */
	...
	unsigned int scatter_gather_enable)
{
	...
	printk(
		C3_KERN_DEBUG "[CDD] User ptr: %x\n",
		(unsigned int)buffer->user_ptr);
#endif
		...
		if ((((unsigned int) buffer->user_ptr) % 4) == 0
			...
			if (_c3_map_user(
				...
				printk(C3_KERN_DEBUG
					...
					" %08x\n",
					(unsigned int)buffer->kernel_ptr);
#endif
	...
	buffer->kernel_ptr = kmalloc(buffer->size, GFP_KERNEL | GFP_DMA);
	if (!buffer->kernel_ptr) {
		printk(
			C3_KERN_ERR
	...
	printk(
		C3_KERN_DEBUG "[CDD] Kernel ptr: %x\n",
		(unsigned int)buffer->kernel_ptr);
...
} /* static unsigned int _c3_user2kernel */
...
} /* static unsigned int _c3_kernel2user */
...
unsigned int prepare_param_list(c3_cdd_param_list_t *k_param_list)
{
	...
	for (j = 0; j < c3_instr_params[i].params_number; j++) {
		...
		} /* if (c3_instr_params[i].params_indexes[j].size_inde... */
		else {
			k_param_list->buffer_list.buffers[j].size
	...
	c3_callback_ptr_t callback, void *callback_param)
{
	...
	while (c3_instr_params[i].fun) {
		if (c3_instr_params[i].instruction ==
			...
			case C3_SP_SEND_RECEIVE_FUNCTION:
				c3_status
					= ((c3_sp_send_receive_function_t)(
						c3_instr_params[i]. fun))(
						(unsigned char *)
						k_param_list->params[0],
						(unsigned int)
			...
			case C3_SP_TIMEOUT_FUNCTION:
				c3_status
					= ((c3_sp_timeout_function_t)(
						c3_instr_params[i]. fun))(
						(unsigned char *)
						k_param_list->params[0],
						callback, callback_param);
			...
			case C3_SP_READ_FUNCTION:
				c3_status
					= ((c3_sp_read_function_t)(
						c3_instr_params[i].fun))(
						(unsigned int)
						k_param_list->params[0],
						(unsigned char *)
			...
			case C3_SP_STANDBY_FUNCTION:
				c3_status = ((c3_sp_standby_function_t)(
					c3_instr_params[i]. fun))(callback,
					callback_param);
				break;

			case C3_SP_WRITE_FUNCTION:
				c3_status
					= ((c3_sp_write_function_t)(
						c3_instr_params[i].fun))(
						(unsigned int)
						k_param_list->params[0],
						(unsigned int)
						...
						callback, callback_param);
				break;
			case C3_GP_LOAD_FIRMWARE_FUNCTION:
				c3_status
					= ((c3_gp_load_firmware_function_t)(
						c3_instr_params[i]. fun))(
						(unsigned char *)
						k_param_list->params[0],
						(unsigned int)
			...
			case C3_GP_DUMP_MEMORY_FUNCTION:
			case C3_GP_RUN_FUNCTION:
				c3_status
					= ((c3_gp_run_function_t)(
						c3_instr_params[i].fun))(
						(unsigned int)
						k_param_list->params[0],
						(unsigned char *)
	...
	} /* while (c3_instr_params[i].function) */

	printk(C3_KERN_ERR "[CDD] Instruction not supported\n");
	return C3_ERR;

} /* static unsigned int execute_program */
	...
	size_t size, loff_t *off_ptr)
{
	...
	} /* if (copy_from_user(&msg, u_param_list, sizeof(msg))) */
	...
	} /* if (down_interruptible (&callback_param.sem) == -EINTR) */
...
} /* static ssize_t _c3_cdd_write_ */

/* ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: */
/* ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: */

static int _c3_cdd_fsync_(struct file *file_ptr, struct dentry *buff_ptr,
	int datasync)
{
	printk(C3_KERN_INFO "[CDD] Fsync\n");
	/* TODO */
	return 0;
...
static int _c3_cdd_ioctl_(struct inode *inode_ptr, struct file *file_ptr,
	unsigned int cmd, unsigned long arg)
{
	c3_ioctl_arg_t k_arg;
	unsigned int err = -EIO;

#ifdef DEBUG
	printk(C3_KERN_DEBUG "[CDD] Device driver \"ioctl\" function\n");
#endif

	if (copy_from_user(&k_arg, (c3_ioctl_arg_t *) arg,
		sizeof(c3_ioctl_arg_t)))
		return -EIO;

	/* if (!copy_from_user (... */

	switch (cmd) {
	case C3_IOCTL_CMD_RECEIVE_MESSAGE:

		if (c3_GP_debug_receive_message(__debug_msg, &k_arg.msg_size)
			== C3_OK) {

			if (!copy_to_user(((c3_ioctl_arg_t *) arg)->msg,
				__debug_msg, k_arg.msg_size) && !copy_to_user(
				(c3_ioctl_arg_t *) arg, &k_arg,
				sizeof(c3_ioctl_arg_t)))
				err = 0;

			/* if (c3_GP_debug_send_message(... */

		} /* if (!copy_from_user (... */

		break;

	case C3_IOCTL_CMD_SEND_MESSAGE:

		if (!copy_from_user(__debug_msg, k_arg.msg, k_arg.msg_size)) {
			if (c3_GP_debug_send_message(__debug_msg,
				k_arg.msg_size) == C3_OK)
				err = 0;
...
static const struct file_operations _c3_device_ops_ = {
	...
	.fsync = _c3_cdd_fsync_,
	.ioctl = _c3_cdd_ioctl_
};
