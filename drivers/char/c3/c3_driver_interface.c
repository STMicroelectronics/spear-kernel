 ...
 * 5. CryptoAPI + SG support for Spear13xx
 */
...
)
{
	...
	C3_START(MPR_CHANNEL_INFO.ids_idx, prgmem, callback, callback_param);

	C3_PREPARE_DMA(c3_dma_0, SN_memory_pool, SN_memory_pool_len,
		       DMA_TO_DEVICE);
	C3_PREPARE_DMA(c3_dma_1, parameters, parameters_len, DMA_TO_DEVICE);
	C3_PREPARE_DMA(c3_dma_2, input, input_len, DMA_TO_DEVICE);
	C3_PREPARE_DMA(c3_dma_3, status, status_len, DMA_FROM_DEVICE);
...
EXPORT_SYMBOL(c3_MPR_load_and_run);
...
)
{
	...
	C3_PREPARE_DMA(c3_dma_0, parameters, parameters_len, DMA_TO_DEVICE);
	C3_PREPARE_DMA(c3_dma_1, input, input_len, DMA_TO_DEVICE);
	C3_PREPARE_DMA(c3_dma_2, status, status_len, DMA_FROM_DEVICE);
...
)
{
	...
	C3_START(RNG_CHANNEL_INFO.ids_idx, /*index,*/ prgmem, callback,
		 callback_param);
...
unsigned int c3_SP_send_receive(
	...
	unsigned int input_len,
	unsigned char *output,
	unsigned int output_len,        /* Make * int pointer? */
	unsigned int standby,
	c3_callback_ptr_t callback,
	void *callback_param
)
{
#if defined(SP_CHANNEL_INFO)

	struct c3_dma_t *c3_dma_0 = NULL;
	struct c3_dma_t *c3_dma_1 = NULL;

	C3_START(SP_CHANNEL_INFO.ids_idx, prgmem, callback, callback_param);

	C3_PREPARE_DMA(c3_dma_0, input, input_len, DMA_TO_DEVICE);
	C3_PREPARE_DMA(c3_dma_1, output, output_len, DMA_FROM_DEVICE);
...
)
{
	...
	C3_PREPARE_DMA(c3_dma_0, timeout, 0x04, DMA_TO_DEVICE);
...
)
{
	...
	C3_PREPARE_DMA(c3_dma_0, value, 0x04, DMA_FROM_DEVICE);
...
{
#if defined(GP_CHANNEL_INFO)

	C3_START(GP_CHANNEL_INFO.ids_idx, prgmem, callback, callback_param);

	/* debug_input_area and debug_output_area are allocated in a DMA-safe
	 * area. I don't need
	 * to invalidate cache lines */

	/* C3 Assembly program */
	GP_SETUP(
		debug_input_phys_area,
		debug_output_phys_area,
		debug_area_size,
		prgmem);
...
)
{
	...
	C3_PREPARE_DMA(c3_dma_0, address, size, DMA_TO_DEVICE);
...
)
{
	...
	C3_PREPARE_DMA(c3_dma_0, params_address,
		params_size, DMA_BIDIRECTIONAL);
...
)
{
	...
	C3_PREPARE_DMA(c3_dma_0, buffer_address, buffer_size, DMA_FROM_DEVICE);
