 ...
 * Error checking
 * No asynchronous hash(ahash) support in the kernel(2.6.32) for IPSec AH mode.
 * Support added in 2.6.34+
 * ...
 */
...
static void callback_aes(
	void *param, unsigned int status)
{
	 ...
	 * The output is placed in the same location and copied back to the
	 * output scatterlist. In case linear memory cannot be allocated
	...
	void *param, unsigned int status)
{
	...
	case 4: /* DIGEST */
		...
		if (!req_ctx->copy) {
			...
			C3_DBG(
				"All SG entries were aligned to 4 bytes, "
				"Normal DMA");
			c3_unmap_sg(
				req_ctx->sg_src, req_ctx->num_src_sg,
				DMA_TO_DEVICE, req_ctx->src_chained);
	...
	unsigned char mode)
{
	...
	if (mode == MODE_RFC3686_CTR) {
		...
		memcpy(
			req_ctx->c3_ivec, ctx->nonce, CTR_RFC3686_NONCE_SIZE);
		memcpy(
			req_ctx->c3_ivec + CTR_RFC3686_NONCE_SIZE, req->info,
			CTR_RFC3686_IV_SIZE);
		...
		*(__be32 *) (req_ctx->c3_ivec + CTR_RFC3686_NONCE_SIZE
		...
		req_ctx->c3_ivec = kmalloc(
			AES_IV_LENGTH, GFP_ATOMIC | GFP_DMA);
		if (!req_ctx->c3_ivec) {
			C3_ERROR(
				"Cant allocate ivec");
			return -ENOMEM;
		...
		memcpy(
			req_ctx->c3_ivec, req->info, AES_IV_LENGTH);
	}
	...
	struct ahash_request *req, unsigned char mode)
{
	...
	req_ctx->c3_ctx = kzalloc(
		C3_DRIVER_DIGEST_CTX_MAX_SIZE, GFP_DMA);
	if (!req_ctx->c3_ctx) {
		C3_ERROR(
			"Mem Alloc failed");
		return -ENOMEM;
