...
struct tasklet_struct c3_tasklet;
...
void c3_acknowledge_request(unsigned long data)
{
	...
	status = programs_buffer[__tasklet_ids_index]
			      .program_queue[programs_buffer[
						     __tasklet_ids_index].
					     start_index].status;
	...
	while (programs_buffer[__tasklet_ids_index].start_index !=
		...
		l = &(((programs_buffer[__tasklet_ids_index].program_queue
			[programs_buffer[__tasklet_ids_index].start_index]).
			   c3_dma_list).list);
		...
		if (programs_buffer[__tasklet_ids_index].program_queue[
			    programs_buffer[__tasklet_ids_index].start_index]
		    .c3_callback) {
...
static unsigned int c3_irq_complete_request(void)
{
...
/* Clear interrupt */
	...
	C3_FINALIZE_PROGRAM(
		programs_buffer[ids_index].program_queue[programs_buffer[
								 ids_index].
							 current_index]);
...
/* Wake up function (user didn't set a callback) */
	if (!(current_program->c3_callback)) {
		if (current_program->sync == 1)
			up(&(current_program->sem));
		else
			current_program->sync = 1;
