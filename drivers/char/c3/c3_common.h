 ...
 * - 2010-09-16 SK: Re-wrote whole DMA handling part
 *  (contiguous buffers and S/G lists)
 * ----------------------------------------------------------------- */
...
/* For dma_alloc_coherent e dma_free_coherent */
...
/*If program->sync=1,the ISR will do an up on sem "program->sem" */	\
				program->sync = 1;			\
			}						\
		}
...
#define C3_END(prgmem)							\
		...
		if (err != C3_OK) {					\
			return err;					\
		} /* if ((err = c3_start_program (... */		\
	} /* if ((err = c3_get_program_memory (program)... */		\
	else {								\
		return err;						\
	} /* else */							\
	if (!program->c3_callback) {					\
		...
		if (down_interruptible(&program->sem) != 0) {		\
			return C3_ERR;					\
		}							\
	}								\
...
#define CACHE_SYNC_AND_SET_SCATTER_LIST(addr, len, dir, dma_c3_t)	\
	do {								\
		...
		addr = (unsigned char *)(((unsigned int)addr) & ~1);	\
		...
		addr = (unsigned char *)((unsigned int)			\
			(((c3_map_info_t *)(addr))->hw_scatter_list)	\
			| 1);						\
		dma_c3_t->cpu_addr = (void *)(((unsigned int)addr) & ~1); \
		...
		dma_c3_t->dma_addr =					\
		dma_map_single(NULL, (void *)(((unsigned int)addr) & ~1), \
			scatter_list_size_in_bytes,			\
			dir) | 1;					\
	} while (0)
...
#define C3_PREPARE_DMA(dma_c3_t, addr_cpu, len, dir)			\
	do {								\
		...
		if ((((unsigned int)addr_cpu) & 1) != 1) {		\
			dma_c3_t->cpu_addr = (void *)addr_cpu;		\
			dma_c3_t->size = len;				\
			dma_c3_t->direction = dir;			\
			dma_c3_t->dma_addr =				\
				dma_map_single(NULL, addr_cpu,		\
					len, dir); \
		}							\
		else {							\
