 ...
 *
 * Copyright (ST) 2012 Deepak Sikri (deepak.sikri@st.com)
 *
...
static void jpeg_abort(s32 error)
{
	...
	/*
	 * SPEAr FIX: We need to reset ip from
	 * misc due to issue: hardware not resetting
	 * jpeg completely.
	 */
	if (g_drv_data->plat_data->plat_reset)
		g_drv_data->plat_data->plat_reset();
...
static s32 designware_jpeg_open(struct inode *inode, struct file *file)
{
	...
	void *dma_filter = g_drv_data->plat_data->dma_filter;
	void *write_data = g_drv_data->plat_data->mem2jpeg_slave;
	void *read_data = g_drv_data->plat_data->jpeg2mem_slave;
...
static int designware_jpeg_resume(struct device *dev)
{
	if (g_drv_data->jpeg_busy[JPEG_READ] ||
			g_drv_data->jpeg_busy[JPEG_WRITE]) {
		int ret = clk_prepare_enable(g_drv_data->clk);
...
static s32 designware_jpeg_probe(struct platform_device *pdev)
{
	...
	g_drv_data->plat_data = plat_data;
...
#ifdef CONFIG_OF
static const struct of_device_id designware_jpeg_of_match[] = {
	{ .compatible = "st,designware-jpeg" },
	{}
};
MODULE_DEVICE_TABLE(of, designware_jpeg_of_match);
#endif
...
#ifdef CONFIG_OF
		.of_match_table = of_match_ptr(designware_jpeg_of_match),
#endif
	},
