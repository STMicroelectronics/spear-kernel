...
static int dwmac1000_dma_init(void __iomem *ioaddr,
				struct stmmac_dma_cfg *dma_cfg , u32 dma_tx,
				u32 dma_rx)
{
	...
	/* Configure the DMA for the stmmac core.
	 * In case the Fixed burst is set through the configurations
	 * from the platform, set up the DMA for fixed burst and
	 * viceversa.
	 * Additionally, set up the DMA PBL (Programmable Burst Length)
	 * Mode. Before stmmac core 3.50 this mode bit was 4xPBL, and
	 ...
	 * depending on pbl value.
	 */
	value = DMA_BUS_MODE_PBL |
		((dma_cfg->pbl << DMA_BUS_MODE_PBL_SHIFT) |
		(dma_cfg->pbl << DMA_BUS_MODE_RPBL_SHIFT));
	 ...
	 * case of GMAC AXI configuration.
	...
	writel(dma_cfg->burst_len_supported, ioaddr + DMA_AXI_BUS_MODE);
