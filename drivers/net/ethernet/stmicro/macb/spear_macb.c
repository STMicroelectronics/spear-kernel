...
static int __init macb_probe(struct platform_device *pdev)
{
	...
	netdev_info(dev, "%s: stmicro cadence MACB at 0x%08lx irq %d (%pM)\n",
			dev->name, dev->base_addr, dev->irq, dev->dev_addr);
	phydev = bp->phy_dev;
	netdev_info(dev, "%s: attached PHY driver [%s] "
