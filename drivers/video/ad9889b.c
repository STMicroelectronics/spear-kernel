...
static int ad9889b_check_edid_status(struct ad9889b_state *state)
{
	...
	struct ad9889b_pdata *pdata = state->pdata;
	int ret = 0, i;
...
void ad9889b_edid_handler(struct work_struct *work)
{
		...
		const struct i2c_device_id *id)
{
	...
	if (!pdata && !np) {
		pr_err("%s: Platform data not found", __func__);
		return -EINVAL;
	}
	...
	if (!pdata && np) {
		...
		if (gpio < 0) {
			...
			return (gpio == -ENODEV) ? -EPROBE_DEFER : gpio;
		}

		pdata = devm_kzalloc(&client->dev, sizeof(*pdata), GFP_KERNEL);
		if (!pdata) {
			dev_err(&client->dev, "kzalloc fail\n");
			return -ENOMEM;
		}

		pdata->irq_gpio = gpio;
		if (of_property_read_u32(np, "fb", &pdata->fb))
			pr_debug("unable to get fb\n");
		if (of_property_read_u32(np, "ain", &pdata->ain))
			pr_debug("unable to get fb\n");
	}
	...
	ret = request_threaded_irq(gpio_to_irq(pdata->irq_gpio),
				...
				IRQF_TRIGGER_FALLING, "ad9889b_isr", client);
	if (ret) {
...
static const struct of_device_id ad9889_hdmi_dt_ids[] = {
...
static struct i2c_driver ad9889b_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = ad9889_hdmi_dt_ids,
#endif
	},
